  ...
  openModal(product?: Product): void {
    if (product) {
      ...
      if (this.currentProduct.entryDate) {
        ...
        const entry = new Date(Number(year), Number(month) - 1, Number(day)); // sin zona horaria
        const formattedDay = entry.getDate().toString().padStart(2, '0');
        const formattedMonth = entry.toLocaleString('es-ES', { month: 'short' }).toLowerCase();
        const formattedYear = entry.getFullYear();
        this.currentProduct.entryDate = `${formattedDay}-${formattedMonth}-${formattedYear}`;
      }
